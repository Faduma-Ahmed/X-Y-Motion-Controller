...
#define NUM_COORDINATES 5

unsigned int X[NUM_COORDINATES];
unsigned int Y[NUM_COORDINATES];

int curr_x = 0;
int curr_y = 0;

int disable_x_forward = 0;
int disable_y_forward = 0;
int disable_x_backward = 0;
int disable_y_backward = 0;
...
//    Init_UART();    //Sets up an echo over a COM port
    ...
    while(1){
        ...
        displayScrollText("BEGIN");

        getCoordinates();

        displayScrollText("PRESS PB");

        while(GPIO_getInputPinValue(SW1_PORT, SW1_PIN) == 1); //Look for rising edge

        displayScrollText("BEGIN MOTION");

        moveMotors();

        displayScrollText("MOTIONS FINISHED");
...
void moveMotors(){
    ...
    for(i = 0; i < NUM_COORDINATES; i++){
        ...
        move_motor_x(i);
        move_motor_y(i);
        displayScrollText("NEXT MOTION");
    }
}

void move_motor_x(int index){
    while(curr_x < X[index]){
        check_x_forward();
        if(disable_x_forward){
            break;
        }
        run_motor_x_forward();
        update_lcd();
    ...
    while(curr_x > X[index]){
        check_x_backward();
        if(disable_x_backward){
            break;
        }
        run_motor_x_backward();
        update_lcd();
    }
}

void check_x_forward(){
...
void check_x_backward(){
    GPIO_setOutputHighOnPin(GPIO_PORT_P8, GPIO_PIN3);
    GPIO_setOutputLowOnPin(GPIO_PORT_P8, GPIO_PIN2);
    GPIO_setOutputHighOnPin(GPIO_PORT_P2, GPIO_PIN5);

    if(GPIO_getInputPinValue(GPIO_PORT_P5, GPIO_PIN1) == 0){
        disable_x_backward = 1;
    } else{
        disable_x_backward = 0;
    }

}

void move_motor_y(int index){
    while(curr_y < Y[index]){
        check_y_forward();
        if(disable_y_forward){
            break;
        }
        run_motor_y_forward();
        update_lcd();
    ...
    while(curr_y > Y[index]){
        check_y_backward();
        if(disable_y_backward){
            break;
        }
        run_motor_y_backward();
        update_lcd();
    }
}

void check_y_forward(){
    GPIO_setOutputHighOnPin(GPIO_PORT_P8, GPIO_PIN3);
...
void check_y_backward(){
    GPIO_setOutputHighOnPin(GPIO_PORT_P8, GPIO_PIN3);
    GPIO_setOutputHighOnPin(GPIO_PORT_P8, GPIO_PIN2);
    GPIO_setOutputHighOnPin(GPIO_PORT_P2, GPIO_PIN5);

    if(GPIO_getInputPinValue(GPIO_PORT_P5, GPIO_PIN1) == 0){
        disable_y_backward = 1;
    } else{
        disable_y_backward = 0;
    }
...
void getCoordinates(){
    int num_entered = 0;
    while(num_entered != NUM_COORDINATES){
//        displayScrollText("HELLO");
        X[num_entered] = getKeypad();
        showChar('0'+X[num_entered], 2 );
//        displayScrollText("HELLO");
        Y[num_entered] = getKeypad();
        showChar('0'+Y[num_entered], 4 );
//        update_lcd(X[num_entered], Y[nuzm_entered]);
        num_entered++;
    }
}

unsigned int getKeypad(){
    GPIO_setOutputLowOnPin(GPIO_PORT_P8, GPIO_PIN3); //set S2 to 0 on multiplexor  we don't need it for keypads
    GPIO_setOutputLowOnPin(GPIO_PORT_P8, GPIO_PIN1);
    GPIO_setOutputLowOnPin(GPIO_PORT_P1, GPIO_PIN1);
    GPIO_setOutputLowOnPin(GPIO_PORT_P1, GPIO_PIN0);
    while(1) //Do this when you want an infinite loop of code
    {
        GPIO_setOutputHighOnPin(GPIO_PORT_P8, GPIO_PIN1); //Col 1
        GPIO_setOutputLowOnPin(GPIO_PORT_P1, GPIO_PIN1);
        GPIO_setOutputLowOnPin(GPIO_PORT_P1, GPIO_PIN0);
        //select input row R1
        GPIO_setOutputLowOnPin(GPIO_PORT_P2, GPIO_PIN5);
        ...
        GPIO_setOutputLowOnPin(GPIO_PORT_P8, GPIO_PIN1);
        GPIO_setOutputHighOnPin(GPIO_PORT_P1, GPIO_PIN1); //Col 2
        GPIO_setOutputLowOnPin(GPIO_PORT_P1, GPIO_PIN0);
        ...
        GPIO_setOutputLowOnPin(GPIO_PORT_P8, GPIO_PIN1); //Col 1
        GPIO_setOutputLowOnPin(GPIO_PORT_P1, GPIO_PIN1);
        GPIO_setOutputHighOnPin(GPIO_PORT_P1, GPIO_PIN0); //Col 3
        //select input row R1
        GPIO_setOutputLowOnPin(GPIO_PORT_P2, GPIO_PIN5);
        GPIO_setOutputLowOnPin(GPIO_PORT_P8, GPIO_PIN2);
...
//        GPIO_setOutputLowOnPin(GPIO_PORT_P1, GPIO_PIN0); //Col 3

//        displayScrollText("NONE");
    }
}

void run_motor_x_forward(){
    int i = 0;
//    displayScrollText("X");
...
void update_lcd(){
    ...
    showChar(curr_x + '0', pos3);
    showChar(curr_y + '0', pos4);
...
void Init_GPIO(void)
{
    ...
    GPIO_setAsOutputPin(GPIO_PORT_P5, GPIO_PIN0|GPIO_PIN2|GPIO_PIN3|GPIO_PIN4|GPIO_PIN5|GPIO_PIN6|GPIO_PIN7);
